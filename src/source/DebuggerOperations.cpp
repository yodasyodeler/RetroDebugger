...
size_t DebuggerOperations::GetOperation(unsigned int address, Operation& operation) {
    const auto opcode1 = DebuggerCallback::ReadMemory(address++);
    ...
    if (m_operations.operations.find(opcode1) != m_operations.operations.end()) {
        ...
        for (const auto& arg : operation.arguments) {
            ...
            for (auto i = 0u; i < argLength; ++i) {
                ...
                arg->operationValue |= DebuggerCallback::ReadMemory(address++) << bitShift;
            }
            re += argLength;
        }
    }
        ...
        const auto opcode2 = DebuggerCallback::ReadMemory(address++);
