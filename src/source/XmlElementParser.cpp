...
static bool StringToBool(const std::string_view& str, bool& value) {
    if (StringNCompare(str, "true")) { value = true; return true; }
    if (StringNCompare(str, "false")) { value = false; return true; }
    return false;
}

static bool StringToRegOperation(const std::string_view& str, RegOperationType& operation) {
    if (StringNCompare(str, "NONE")) { operation = RegOperationType::NONE; return true; }
    if (StringNCompare(str, "POSTINC")) { operation = RegOperationType::POSTINC; return true; }
    if (StringNCompare(str, "POSTDEC")) { operation = RegOperationType::POSTDEC; return true; }
    if (StringNCompare(str, "PREINC")) { operation = RegOperationType::PREINC; return true; }
    if (StringNCompare(str, "PREDEC")) { operation = RegOperationType::PREDEC; return true; }
    if (StringNCompare(str, "REG_OFFSET_ADD")) { operation = RegOperationType::REG_OFFSET_ADD; return true; }
    if (StringNCompare(str, "REG_OFFSET_SUB")) { operation = RegOperationType::REG_OFFSET_SUB; return true; }
    if (StringNCompare(str, "REG_OFFSET_U8BIT")) { operation = RegOperationType::REG_OFFSET_U8BIT; return true; }
    if (StringNCompare(str, "REG_OFFSET_U16BIT")) { operation = RegOperationType::REG_OFFSET_U16BIT; return true; }
    if (StringNCompare(str, "REG_OFFSET_S8BIT")) { operation = RegOperationType::REG_OFFSET_S8BIT; return true; }
    if (StringNCompare(str, "REG_OFFSET_S16BIT")) { operation = RegOperationType::REG_OFFSET_S16BIT; return true; }
    return false;
}
...
static bool StringToImmediateArgType(const std::string_view& str, ArgumentType& type) {
    if (StringNCompare(str, "R8")) { type = ArgumentType::S8BIT; return true; }
    if (StringNCompare(str, "S8BIT")) { type = ArgumentType::S8BIT; return true; }
    if (StringNCompare(str, "SIGNED8BIT")) { type = ArgumentType::S8BIT; return true; }
    if (StringNCompare(str, "R16")) { type = ArgumentType::S16BIT; return true; }
    if (StringNCompare(str, "S16BIT")) { type = ArgumentType::S16BIT; return true; }
    if (StringNCompare(str, "SIGNED16BIT")) { type = ArgumentType::S16BIT; return true; }
    if (StringNCompare(str, "D8")) { type = ArgumentType::U8BIT; return true; }
    if (StringNCompare(str, "A8")) { type = ArgumentType::U8BIT; return true; }
    if (StringNCompare(str, "U8BIT")) { type = ArgumentType::U8BIT; return true; }
    if (StringNCompare(str, "UNSIGNED8BIT")) { type = ArgumentType::U8BIT; return true; }
    if (StringNCompare(str, "D16")) { type = ArgumentType::U16BIT; return true; }
    if (StringNCompare(str, "A16")) { type = ArgumentType::U16BIT; return true; }
    if (StringNCompare(str, "U16BIT")) { type = ArgumentType::U16BIT; return true; }
    if (StringNCompare(str, "UNSIGNED16BIT")) { type = ArgumentType::U16BIT; return true; }
    return false;
}

static bool StringToArgType(const std::string_view& str, ArgumentType& type) {
    if (StringNCompare(str, "REG")) { type = ArgumentType::REG; return true; }
    if (StringNCompare(str, "CONST")) { type = ArgumentType::CONSTANT; return true; }
    if (StringNCompare(str, "CONSTANT")) { type = ArgumentType::CONSTANT; return true; }
    if (StringNCompare(str, "COND")) { type = ArgumentType::CONDITIONAL; return true; }
    if (StringNCompare(str, "CONDITIONAL")) { type = ArgumentType::CONDITIONAL; return true; }
...
static bool StringToOpcodeLength(const std::string_view& str, OpcodeLength& opcodeLength) {
    if (StringNCompare("8", str)) { opcodeLength = 8; return true; }
    if (StringNCompare("BYTE", str)) { opcodeLength = 8; return true; }
    if (StringNCompare("16", str)) { opcodeLength = 16; return true; }
    if (StringNCompare("32", str)) { opcodeLength = 32; return true; }
    return false;
}
...
bool ParseArgOperations(std::string& name, XmlDebuggerArgument& arg) {
    ...
    if (operationFind == std::string::npos) { arg.operation = RegOperationType::NONE; return false; }

    if (operationFind == 0) { //PreOp
        if (value[operationFind] == '+' && IsAlphaString(value.substr(1))) { arg.operation = RegOperationType::PREINC; }
        else if (value[operationFind] == '-') { arg.operation = RegOperationType::PREDEC; }
        else { arg.operation = RegOperationType::NONE; return false; }
        name.erase(0, 1);
    }
    else if (operationFind == value.size() - 1) { //PostOp
        if (value[operationFind] == '+') { arg.operation = RegOperationType::POSTINC; }
        else if (value[operationFind] == '-') { arg.operation = RegOperationType::POSTDEC; }
        else { arg.operation = RegOperationType::NONE; return false; }
        name.erase(name.size() - 1);
    }
        ...
        if (IsAlphaString(firstParam) && StringToUnsigned(secondParam, arg.value.offset)) { //TODO: is there a unit test for this?
            ...
            else if (immediateType == ArgumentType::U16BIT) { arg.operation = RegOperationType::REG_OFFSET_U16BIT; }
            else if (immediateType == ArgumentType::S8BIT) { arg.operation = RegOperationType::REG_OFFSET_S8BIT; }
            else if (immediateType == ArgumentType::S16BIT) { arg.operation = RegOperationType::REG_OFFSET_S16BIT; }
            else { return false; }
            name = firstParam;
        }
    ...
    if (name.find(reservedCharacters) != std::string::npos) { arg.operation = RegOperationType::NONE; return false; }
...
bool XmlElementParser::ParseXmlElement(const tinyxml2::XMLElement* element, XmlDebuggerOperations& operations) {
    ...
    else if (!StringToUnsigned(str, operations.extendedOpcode)) { return SetLastError(element, ErrorObtainingUnsignedInt + extenededOpcodeStr); }
...
bool XmlElementParser::ParseXmlElement(const tinyxml2::XMLElement* element, XmlDebuggerOperation& operation) {
    ...
    if (str) {
        operation.isJump = (stricmp(str, trueStr) == 0);
    }
...
bool XmlElementParser::ParseXmlElement(const tinyxml2::XMLElement* element, XmlDebuggerArgument& argument) {
    ...
    if (str) { argument.value.reg = str; isCandidateForAuto = false; }
