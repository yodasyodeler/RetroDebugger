...
enum class ArgumentType : int { UNKOWN = 0, AUTO, CONSTANT, REG, CONDITIONAL, S8BIT, S16BIT, S32BIT, U8BIT, U16BIT, U32BIT };

enum class RegOperationType : int { NONE = 0, POSTINC, POSTDEC, PREINC, PREDEC, REG_OFFSET_ADD, REG_OFFSET_SUB, REG_OFFSET_U8BIT, REG_OFFSET_U16BIT, REG_OFFSET_S8BIT, REG_OFFSET_S16BIT };

typedef unsigned int OpcodeLength;

//TODO: is the Reset method a good idea?
struct XmlArgValue {
    void Reset() { nameFirst = true; name.clear(); reg.clear(); offset = 0; }
...
struct XmlDebuggerArgument {
    void Reset() { type = ArgumentType::UNKOWN; indirectArg = false; operation = RegOperationType::NONE; value.Reset(); }
    ArgumentType type;
    bool indirectArg;
...
struct XmlDebuggerOperation {
    void Reset() { opcode = 0u; command.clear(); arguments.clear(); isJump = false; }
    unsigned int opcode;
    std::string command;
...
struct XmlDebuggerOperations {
    void Reset() { opcodeLength = 0; extendedOpcode = NORMAL_OPERATIONS_KEY; operations.clear(); }
...
enum class ImmediateType : int { UNKOWN = -1, NONE = 0 }; //TODO: think about naming for this enum
enum class FlagType : int { UNKOWN = -1, NONE = 0, ZERO, SUBTRACT, HALF_CARRY, CARRY };
enum class OperationType : int { UNKOWN = -1, NONE = 0, NOP, LOAD, ADD };
...
struct Argument {
    inline bool operator==(const Argument& rhs) const {
        return (type == rhs.type) && (indirectArg == rhs.indirectArg) && (operation == rhs.operation) &&
            (operationValue == rhs.operationValue) && (name == rhs.name);
    }
    ArgumentType type;
    bool indirectArg;
    RegOperationType operation;
...
struct OperationInfo {
    OperationInfo(std::string Name, bool IsJump) : name(Name), isJump(IsJump) {}
    std::string name;
    bool isJump;
};
...
inline unsigned int GetArgTypeLength(const ArgumentType& argType) {
    ...
    else if (argType == ArgumentType::U8BIT) { return 1u; }
    else if (argType == ArgumentType::S16BIT) { return 2u; }
    else if (argType == ArgumentType::U16BIT) { return 2u; }
    else if (argType == ArgumentType::S32BIT) { return 4u; }
    else if (argType == ArgumentType::U32BIT) { return 4u; }
    else { return 0u; } //TODO: should this make it fail?
}
