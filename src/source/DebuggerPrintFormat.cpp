...
static std::string to_string(int value, int length, bool isHex) {
    ...
    else if (length > 8) { width = isHex ? 4 : 5; }
    else { width = isHex ? 2 : 3; }
...
std::string PrintBreakInfo(const std::map<BreakNum, BreakInfo>& breakInfo) {
    ...
    sprintf_s(buffer, "%-8s""%-15s""%-5s""%-4s""%-19s""%s""\n", Num, Type, Disp, Enb, Address, What);
    re += buffer;
    for (const auto& info : breakInfo) {
        ...
        sprintf_s(buffer, "%-8u""%-15s""%-5s""%-4s""0x%016X ""%s""\n",
                  info.first, BreakTypeToString.at(info.second.type).c_str(), BreakDispToString.at(info.second.disp).c_str(), (info.second.isEnabled ? "y" : "n"), info.second.address, what.c_str());
        re += buffer;
...
std::string PrintInstructions(const std::map<unsigned int, Operation>& commandInfo) {
    ...
    for (const auto& info : commandInfo) {
        std::stringstream msg;
        msg << to_string(info.first, 16, true) << "  " << info.second.info->name << "\t  "; //TODO: opcodeLength is not accounted for

        //TODO: need to rethink this. Can't assume the memory value that is read from the callback.
        temp += msg.str();

        const uint8_t byte = DebuggerCallback::ReadMemory(info.first + 1);
        const uint16_t word = (DebuggerCallback::ReadMemory(info.first + 2) << 8) | byte;
        ...
        for (auto& arg : info.second.arguments) {
            ...
            else if (arg->type == ArgumentType::S16BIT) { temp += to_string(word, true); }
            //else if (arg->type == ArgumentType::S32BIT) {}
            else if (arg->type == ArgumentType::U8BIT) { temp += to_string(byte, true); }
            else if (arg->type == ArgumentType::U16BIT) { temp += to_string(word, true); }
