...
std::tuple<Expr::IExprPtr, ErrorsPtr, std::shared_ptr<DebuggerCallbacks>> ScanAndParse(std::string_view source) {
    ...
    return std::tuple<Expr::IExprPtr, ErrorsPtr, std::shared_ptr<DebuggerCallbacks>>{ expr, errors, std::make_shared<DebuggerCallbacks>() };
}
